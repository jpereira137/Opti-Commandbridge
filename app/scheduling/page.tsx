...
export default function Scheduling() {
  const { employees, shifts, isLive, isSyncing, sync } = useConnecteam()
  ...
  const filteredEmployees = employees.filter(e=>(ctFilter==="all"||e.connecteamAccount===ctFilter)&&e.status!=="onboarding")
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.5rem"}}>
        <div>
          ...
          <div className="page-sub">Week of May 25 – 31, 2026 · {isLive ? "Live data from Connecteam" : "Demo data"}</div>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={sync} disabled={isSyncing} className="btn btn-sm" style={{gap:6}}>
      ...
      <div style={{display:"flex",gap:10,marginBottom:16}}>
        {(["A","B"] as const).map(ct=>(
          <div key={ct} style={{display:"flex",alignItems:"center",gap:8,background:"#fff",border:"1px solid #e8ecf4",borderRadius:10,padding:"8px 14px",fontSize:13}}>
            <div style={{width:7,height:7,borderRadius:"50%",background:isLive?"#16a34a":"#f59e0b"}}/>
            ...
            <span className={`badge ${isLive ? "badge-green" : "badge-amber"}`} style={{fontSize:10}}>{isLive ? "Live" : "Demo"}</span>
          </div>
        ))}
      </div>
          ...
          <div key={emp.id} style={{display:"grid",gridTemplateColumns:"140px repeat(7,1fr)",borderBottom:"1px solid #f1f5f9"}}>
            <div style={{padding:"10px 12px",borderRight:"1px solid #e8ecf4",display:"flex",alignItems:"center",gap:8}}>
              ...
              <div>
                ...
                <div style={{fontSize:10,color:"#94a3b8"}}>{emp.role?.split(" ")[0] || "Employee"}</div>
              </div>
            </div>
            {DATES.map(date=>{
